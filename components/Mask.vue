...
const props = defineProps<{
  ...
  // ('#222', 'rgb(...)', 'oklch(...)', or a CSS var like 'var(--fenbrook-accent)').
  // Composes with `variant`: 'dim' becomes 70% of this color, 'blur' 25% + blur.
  // Defaults to `var(--fenbrook-bg)`.
  color?: string
}>()
...
.annotated-mask {
  position: absolute;
  background: var(--mask-color, var(--fenbrook-bg));
  pointer-events: none;
}

.annotated-mask.annotated-mask-dim {
  background: color-mix(in oklab, var(--mask-color, var(--fenbrook-bg)) 70%, transparent);
}

.annotated-mask.annotated-mask-blur {
  background: color-mix(in oklab, var(--mask-color, var(--fenbrook-bg)) 25%, transparent);
