...
const gridStyle = computed(() => ({
  ...
  gap: props.gap,
  '--fenbrook-gallery-radius': props.bleed ? '0px' : props.radius,
  '--fenbrook-gallery-fit': props.fit,
}))
</script>

<template>
  <div class="slidev-layout fenbrook-gallery" :class="{ bleed }">
...
.gallery-cell :deep(img) {
  ...
  min-height: 0;
  height: auto;
  object-fit: var(--fenbrook-gallery-fit, cover);
  border-radius: var(--fenbrook-gallery-radius, 6px);
  display: block;
}
...
.overlay-card {
  pointer-events: auto;
  background: color-mix(in oklab, var(--fenbrook-bg) 88%, transparent);
