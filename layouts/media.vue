...
<template>
  <div
    class="slidev-layout fenbrook-media"
    :class="[
      `side-${side}`,
      bleed ? 'bleed' : 'padded',
      `caption-${captionSide}`,
...
.fenbrook-media.padded {
  padding: 3rem 4rem 3.5rem;
}

.fenbrook-media.bleed {
  padding: 3rem 0 3.5rem 4rem;
  gap: 3.5rem;
}

.fenbrook-media.side-left {
  grid-template-columns: 1.15fr 1fr;
}
.fenbrook-media.side-left .media-text { order: 2; }
.fenbrook-media.side-left .media-column { order: 1; }
.fenbrook-media.side-left.bleed {
...
.fenbrook-media.caption-right .media-column {
  flex-direction: row;
  align-items: stretch;
  gap: 0.9rem;
}

.fenbrook-media.caption-left .media-column {
...
.media-frame {
  ...
  overflow: hidden;
  border-radius: 6px;
  background: color-mix(in oklab, var(--fenbrook-fg) 4%, transparent);
}
...
.fenbrook-media.bleed .media-frame {
  border-radius: 0;
...
.fenbrook-media.caption-right .media-caption,
.fenbrook-media.caption-left .media-caption {
  margin-top: 0;
  align-self: flex-end;
  max-width: 12rem;
  text-align: left;
